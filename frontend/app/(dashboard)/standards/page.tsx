...
export default function StandardsPage() {
  ...
  async function createStandard() {
    ...
    try {
      const res = await fetch(`${API_BASE}/standards/`, { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ code, title, type, version }) });
    ...
    <div className="min-h-full bg-slate-50 p-8 text-slate-900">
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-7 flex items-start justify-between gap-6">
          <div><h1 className="text-2xl font-semibold tracking-tight">Standards</h1><p className="mt-1 text-sm text-slate-500">Manage compliance standards, versions and organizational applicability.</p></div>
          <button onClick={() => setOpen(true)} className="shrink-0 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-300">+ Add Standard</button>
        </div>
        ...
        {!loading && !error && <>
          <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b border-slate-200 bg-slate-50"><tr>
            <th className="px-5 py-4 text-left font-semibold text-slate-700">Standard</th><th className="px-5 py-4 text-left font-semibold text-slate-700">Version</th><th className="px-5 py-4 text-left font-semibold text-slate-700">Assessment Model</th><th className="px-5 py-4 text-left font-semibold text-slate-700">Status</th><th className="px-5 py-4 text-center font-semibold text-slate-700">Controls</th><th className="px-5 py-4 text-left font-semibold text-slate-700">Scope</th><th className="px-5 py-4 text-right font-semibold text-slate-700">Actions</th>
          </tr></thead><tbody className="divide-y divide-slate-100">
            {pagedItems.map((standard) => { const summary = matrixSummary[standard.id]; const isDraft = standard.status !== "published" && standard.status !== "archived"; return <tr key={standard.id} className="hover:bg-slate-50/70">
              ...
              <td className="px-5 py-5"><div className="flex justify-end gap-2">
                ...
                <button onClick={() => router.push(`/matrix?standard_id=${standard.id}`)} disabled={!summary?.ready} className="rounded-lg bg-slate-900 px-3 py-2 text-xs font-medium text-white hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400" title={!summary?.ready ? "Build the compliance matrix first" : "Open compliance matrix"}>{summary?.ready ? "View Matrix" : "Build Matrix"}</button>
      ...
      {open && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4"><div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl"><h2 className="text-xl font-semibold text-slate-900">Create New Standard</h2><p className="mt-1 text-sm text-slate-500">Create the standard definition first. Matrix and activation are completed during setup.</p><div className="mt-6 space-y-5">
      ...
      </div><div className="mt-7 flex justify-end gap-3"><button onClick={() => setOpen(false)} className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50">Cancel</button><button onClick={createStandard} className="rounded-lg bg-slate-900 px-5 py-2.5 text-sm font-medium text-white hover:bg-slate-800">Create Standard</button></div></div></div>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            ...
            <h2 className="mt-1 text-xl font-semibold text-slate-900">
              Activate {activateStandard.title || activateStandard.code}
            </h2>

            <p className="mt-1 text-sm text-slate-500">
              Publish the configured version so it becomes the active compliance standard.
            </p>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm">
              <div>
                ...
                <div className="mt-1 font-medium">{activateStandard.code}</div>
              </div>

              <div>
                <div className="text-xs text-slate-500">Version</div>
                <div className="mt-1 font-medium">
                  {activateStandard.version || "-"}
                </div>
              </div>

              <div>
                <div className="text-xs text-slate-500">Assessment Model</div>
                <div className="mt-1 font-medium">
                  {assessmentLabel(activateStandard.type)}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-1 font-medium">
                  {matrixSummary[activateStandard.id]?.controls ?? "-"}
                </div>
              </div>
            </div>

            <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
              Organizational scope is not defined in the current standard setup contract.
              Activation publishes the configured version; scope management remains a separate setup step.
            </div>

            <div className="mt-7 flex justify-end gap-3">
              <button
                onClick={() => setActivateStandard(null)}
                disabled={activating}
