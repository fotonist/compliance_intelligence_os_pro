...
import ProcessRiskLinkModal from "../../../../components/ProcessRiskLinkModal";
...
export default function ProcessDetailPage() {
  ...
  const [derivedStandards, setDerivedStandards] = useState<any[]>([]);
...
// load available risks when link modal opens
useEffect(() => {
  if (!linkOpen || !processId) return;
  ...
  async function loadAvailableRisks() {
    try {
      ...
      setAvailableRisks(Array.isArray(json) ? json : []);
    } catch (err) {
...
async function refreshDerivedStandards(pid: number) {
  try {
    setStandardsLoading(true);

    const res = await apiFetch(`/company/processes/${pid}/standards`, {
      method: "GET",
    });
    ...
    const json = await res.json();
    setDerivedStandards(Array.isArray(json) ? json : []);
  } catch (err) {
    console.error("Failed to load derived standards", err);
    setDerivedStandards([]);
  } finally {
...
await refreshDerivedStandards(pid);
  }
/* ============================= */
/* PROCESS TASKS */
/* ============================= */

async function refreshTasks(pid: number) {
  try {
    setTasksLoading(true);

    const res = await apiFetch(
      `/company/tasks/process/${pid}`,
      { method: "GET" }
    );
  ...
  async function save() {
    ...
    try {
      ...
      await refreshDerivedStandards(processId);
  ...
  async function doLinkRisk() {
   ...
   if (!selectedRiskId) {
  setRiskUiError("Select a risk.");
  return;
}

const rid = selectedRiskId;
  ...
  actions={
    <>
      <Button
        variant="secondary"
        onClick={() =>
          router.push(`/company/tasks/create?process_id=${processId}`)
        }
      >
        + Create Task
      </Button>
        ...
        >
          ...
          <div className="col-span-2 text-right">
            <Button
              variant="ghost"
              onClick={() =>
                router.push(`/company/tasks/${t.id}`)
              }
            >
              Open
            </Button>
