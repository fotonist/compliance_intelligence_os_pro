    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-5">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-5 items-end">
          <div><div className="text-xs text-slate-400 mb-2">Audit Scope Process</div><select value={processId ?? ""} onChange={(e) => setProcessId(e.target.value ? Number(e.target.value) : null)} disabled={loadingProcesses} className="w-full rounded-lg bg-slate-950 border border-slate-800 px-3 py-2.5 text-sm text-slate-100 outline-none focus:border-slate-600"><option value="">Select process...</option>{processes.map((process) => <option key={process.id} value={process.id}>{process.code} — {process.name}</option>)}</select></div>
