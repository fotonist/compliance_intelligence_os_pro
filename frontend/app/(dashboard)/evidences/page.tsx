    ...
    <div className="min-h-full bg-[#f7f9fc] px-6 py-8 text-slate-950">
      <div className="mx-auto max-w-[1440px] space-y-7">
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <button onClick={() => router.push("/evidences/create")} className="inline-flex items-center justify-center rounded-xl bg-[#0b5cff] px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-blue-600/20 transition hover:bg-[#084ed6] focus:outline-none focus:ring-4 focus:ring-blue-500/15">+ Create Evidence</button>
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1100px] text-left text-sm">
              <thead className="border-b border-slate-200 bg-slate-50/80 text-[11px] font-bold uppercase tracking-[0.08em] text-slate-500">
                <tr>{["Evidence","Standard","Requirement","Control","Status","Coverage","Files","Risks","Action"].map((h) => <th key={h} className="px-5 py-3.5">{h}</th>)}</tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                ...
                {!loading && filtered.map((r) => (
                  <tr key={r.evidence_id} className="transition-colors hover:bg-slate-50/70">
                    ...
                    <td className="px-5 py-4 text-slate-600">{r.standard?.code || "—"}</td>
                    <td className="px-5 py-4 text-slate-600">{r.requirement?.code || "—"}</td>
                    <td className="px-5 py-4 text-slate-600">{r.control?.code || "—"}</td>
                    ...
                    <td className="px-5 py-3.5"><button onClick={() => router.push(`/evidences/${r.evidence_id}`)} className="inline-flex items-center rounded-lg px-2.5 py-1.5 text-sm font-semibold text-blue-600 transition hover:bg-blue-50 hover:text-blue-800">Open →</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
