...
return `flex items-center gap-2 px-4 py-2 rounded text-sm hover:bg-slate-800 ${
      pathname.startsWith(href) ? "bg-slate-800 font-medium" : ""
    }`;
}
...
return ( <aside className="w-64 bg-[#020817] border-r border-slate-800 min-h-screen px-3 py-4 text-slate-200 flex flex-col"> <div> <div className="mb-6 px-2"> <div className="text-lg font-bold">Compliance OS</div> <div className="text-xs text-slate-400">
Governance & Intelligence Engine </div> </div>
    ...
    <nav className="space-y-3">
      ...
      >
        ...
        <Link href="/standards" className={itemClass("/standards")}>
          Standards
        </Link>
      </Section>
      ...
      <Section
        ...
        toggle={toggle}
      >
        <Link href="/matrix" className={itemClass("/matrix")}>
          Control Matrix
        </Link>

        <Link
          href="/matrix/intelligence"
          className={itemClass("/matrix/intelligence")}
        >
          Control Analytics
        </Link>
...
/>
        ...
        <Link
          ...
          className={itemClass("/intelligence/readiness/processes")}
        >
          Executive Readiness
        </Link>

        <Link href="/risks" className={itemClass("/risks")}>
        ...
        <Link
          ...
          className={itemClass("/intelligence/control/control-health")}
        >
          Control Health
        </Link>
      </Section>
...
/>
      ...
      >
		...
		<Link href="/admin/licenses" className={itemClass("/admin/licenses")}>
              License Management
        </Link>

        <Link href="/admin/logs" className={itemClass("/admin/logs")}>
          Audit Logs
        </Link>
      </Section>
    </nav>
    ...
    ">
      ...
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-semibold">
          {currentUser?.full_name || currentUser?.username || "Logged in"}
        </span>
      </div>
      ...
      {(currentUser?.email ||
        ...
        <div className="mt-1 text-xs text-slate-300 space-y-0.5">
          ...
          {currentUser?.role && (
            <div className="text-slate-400">Role: {currentUser.role}</div>
          )}
          {currentUser?.tenant_id != null && (
...
{!!badge && badge > 0 && ( <span className="bg-red-600 text-white text-xs px-2 py-0.5 rounded-full animate-pulse">
{badge} </span>
)} </div>
