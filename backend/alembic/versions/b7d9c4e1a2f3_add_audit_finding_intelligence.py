...
def upgrade():
    ...
    # ---------------------------------------------------------
    # Finding pressure is calculated at control/tenant level so an audit
    # finding affects the same control that it was raised against.
    ...
    # RISK EXPOSURE VIEW
    # ---------------------------------------------------------
    # Preserve the existing evidence coverage calculation and add the
    # active audit-finding pressure for the risk's control.
    op.execute("""
    ...
    op.execute("""
        ...
        SELECT
            ...
            (
                SELECT COUNT(*)
                FROM audit_finding_records afr
                WHERE afr.tenant_id = ei.tenant_id
            ) AS total_findings,
            ...
            (
                ...
                FROM audit_finding_records afr
                WHERE afr.tenant_id = ei.tenant_id
                  AND UPPER(COALESCE(afr.severity, '')) IN ('CRITICAL', 'MAJOR', 'HIGH')
                  AND UPPER(COALESCE(afr.status, '')) NOT IN ('CLOSED', 'VERIFIED')
            ) AS high_severity_open_findings,
            ...
            (
                ...
                FROM audit_finding_records afr
                WHERE afr.tenant_id = ei.tenant_id
                  AND UPPER(COALESCE(afr.severity, '')) = 'CRITICAL'
                  AND UPPER(COALESCE(afr.status, '')) NOT IN ('CLOSED', 'VERIFIED')
            ) AS critical_open_findings

        FROM analytics.v_evidence_intelligence ei
        GROUP BY tenant_id;
    """)


def downgrade():
    # Restore the original analytics views from the previous migration.
