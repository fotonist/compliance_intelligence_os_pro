...
def upgrade():
    # Finding intelligence is added as a dedicated source and then consumed
    # by the risk exposure layer. Do not replace v_dashboard_summary here:
    # the production Executive Intelligence service depends on its canonical
    # KPI contract (unified_exposure, compliance_health, risk_pressure, etc.).
    op.execute("""
    ...
    # Keep the existing v_risk_exposure contract and extend it with finding
    # pressure. This is consumed by UEEEngine as well as other analytics APIs.
    op.execute("""
