...
TISAX_STANDARD_VERSION_CODE = "AL2"
...
BASE_CRITERIA: List[Dict[str, object]] = [
    {
        ...
        "al1": "Define basic security responsibilities and policies.",
    ...
    },
    {
        ...
        "al1": "Identify key assets and basic scope.",
    ...
    },
    {
        ...
        "al1": "Provide basic awareness training for relevant staff.",
    ...
    },
    {
        ...
        "al1": "Restrict physical access to critical areas.",
    ...
    },
    {
        ...
        "al1": "Basic operational procedures and change tracking.",
    ...
    },
    {
        ...
        "al1": "Basic access control and least privilege.",
    ...
    },
    {
        ...
        "al1": "Define basic supplier security expectations.",
    ...
    },
    {
        ...
        "al1": "Define a simple incident reporting and handling process.",
    ...
    },
    {
        ...
        "al1": "Basic continuity planning for critical services.",
    ...
    },
    {
        ...
        "al1": "Basic secure development guidance for relevant teams.",
...
def seed_tisax_isa06(db: Session) -> None:
    # ------------------------------------------------------------------
    # 1) Ensure canonical TISAX standard
    # ------------------------------------------------------------------
    std = db.execute(
        text(
            """
        ...
        db.execute(
            text(
                ...
                INSERT INTO standards
                    (code, title, description, type, version)
                VALUES
                    (:code, :title, :description, :type, :version)
                """
            ),
    ...
    # ------------------------------------------------------------------
    # 2) Resolve canonical TISAX version
    # ------------------------------------------------------------------
    version = db.execute(
        text(
            """
        ...
        {
            "standard_id": std_id,
            "version_code": TISAX_STANDARD_VERSION_CODE,
        },
    ).fetchone()

    if not version:
        raise RuntimeError(
            "TISAX canonical version 'AL2' was not found."
        )

    version_id = int(version[0])

    # ------------------------------------------------------------------
    # 3) Process areas bound to the canonical StandardVersion
    # ------------------------------------------------------------------
    ...
    for code, name, sort_order in PROCESS_AREAS:
        row = db.execute(
            text(
                """
                SELECT id
                FROM standard_process_areas
                WHERE standard_version_id = :standard_version_id
                  AND code = :code
                ORDER BY id
                LIMIT 1
                """
        ...
        if not row:
            db.execute(
                text(
                    """
                    INSERT INTO standard_process_areas
                        (
                            standard_id,
                            standard_version_id,
                            code,
