...
# 🔐 ENTERPRISE ACCESS LAYER (Board-only)
...
def _control_ai_priority(
    ...
    """
    Production-level GAP AI priority.

    Rationale:
    - worst_severity: strongest primary signal
    - avg_exposure: residual exposure pressure
    - avg_escalation: forward-looking deterioration risk
    - gap_count: density / breadth of control weakness
    """
...
def get_intelligence_overview(
    ...
    if risk_ids:
        ...
        pr_rows = db.execute(pr_stmt).all()
        for rid, pid, pname in pr_rows:
            rid_int = int(rid)
    ...
    # ---------------------------------------------------
    # Phase-2 Exposure Engine (Unified Brain)
    # ---------------------------------------------------
    exposure_by_risk_id: Dict[int, float] = {}
    unified_by_risk_id: Dict[int, float] = {}
    rv_by_risk_id: Dict[int, int] = {}

    try:
        engine = ExposureEngine(db)
        engine_rows = engine.compute_risk_exposure(tenant_id=tenant_id, limit=1000000)
        for row in engine_rows:
            ...
            rv_by_risk_id[int(row.risk_id)] = int(row.risk_version_id)
    except Exception:
        # Engine fail-safe: degrade gracefully (prob-only ranking)
        exposure_by_risk_id = {}
        unified_by_risk_id = {}
        rv_by_risk_id = {}
    ...
    summary = IntelligenceSummary(
        total_risks=int(total_risks or 0),
    ...
    # ---------------------------------------------------
    # Top risks: Phase-2 Unified score (engine) fallback prob-only
    # ---------------------------------------------------
    scored_rows = []
    for f, r, c in latest_rows:
        ...
        unified = float(unified_by_risk_id.get(rid, 0.0))
        if unified <= 0.0:
            unified = prob
    ...
    top_risks_rows = scored_rows

    top_risks: List[IntelligenceTopRisk] = []
    for _unified, _exposure, f, r, c in top_risks_rows:
        rid = int(r.id)
    ...
    # ---------------------------------------------------
    # Top controls aggregation (use unified score)
    # ---------------------------------------------------
    ...
    for unified, exposure, f, r, c in scored_rows:
        ...
        legacy_ai = _risk_ai_priority(prob, delta)

        node = control_agg.get(int(cid))
        if not node:
            node = {
                "control_id": int(cid),
                "control_code": getattr(c, "code", None) if c else None,
            ...
            control_agg[int(cid)] = node

        node["risk_count"] += 1
        node["prob_sum"] += prob
        node["prob_max"] = max(node["prob_max"], prob)
        node["delta_sum"] += delta
        node["legacy_ai_sum"] += legacy_ai
        node["unified_sum"] += float(unified)

    top_controls_list: List[IntelligenceTopControl] = []
    for node in control_agg.values():
        rc = int(node["risk_count"]) if node["risk_count"] else 0
    ...
    for unified, exposure, f, r, c in scored_rows:
        ...
        if exposure <= 0:
            if prob < 0.80:
                continue
            if _risk_level_rank(getattr(r, "risk_level", None)) < 3:
                continue
        else:
            if prob < 0.75:
                continue
...
def get_intelligence_dashboard(
    ...
    row = db.execute(stmt, {"tenant_id": tenant_id}).mappings().first()
...
def get_intelligence_evidence(
    ...
    res = db.execute(stmt, {"tenant_id": tenant_id, "limit": int(limit)})
...
def get_risk_exposure(
    ...
    engine = ExposureEngine(db)
    rows = engine.compute_risk_exposure(tenant_id=tenant_id, limit=int(limit))
...
def get_gap_intelligence(
    ...
    exposure_engine = ExposureEngine(db)
    ...
    for row in gap_rows:
        ...
        control_node = control_map[cid]

        control_node["gap_count"] += 1


        sev = float(row["severity_score"] or 0.0)

        control_node["worst_severity"] = max(
            float(control_node["worst_severity"]),
            sev
        )


        if rid not in control_node["risks"]:

            exposure = exposure_map.get(int(rid))

            control_node["risks"][rid] = {
                "risk_id": rid,
                "risk_title": row["risk_title"],
                "risk_level": row["risk_level"],
                "exposure_score": (
                    float(exposure.residual_exposure)
                    if exposure else 0.0
                ),
                "escalation_probability": (
                    float(exposure.escalation_probability_30d)
                    if exposure else 0.0
                ),
                "expected_score_delta": (
                    float(exposure.expected_score_delta)
                    if exposure else 0.0
                ),
                "gap_count": 0,
                "worst_severity": 0.0,
        ...
        risk_node = control_node["risks"][rid]

        risk_node["gap_count"] += 1

        risk_node["worst_severity"] = max(
            float(risk_node["worst_severity"]),
            sev
        )
    ...
    controls_out.sort(
        key=lambda x: float(x["ai_priority_score"]),
        reverse=True
    )
    ...
    uncovered = sum(
        1
        for x in gap_rows
        if (x.get("status") or "").lower() == "uncovered"
    )

    partial = sum(
        1
        for x in gap_rows
        if (x.get("status") or "").lower() == "partial"
    )
...
def get_control_health(
    ...
    high_risks = (
        db.query(func.count(Risk.id))
         .filter(
        Risk.tenant_id == tenant_id,
        func.lower(Risk.risk_level) == "high"
    )
        .scalar()
    ) or 0
    ...
    # Latest forecast avg escalation probability
    ...
    try:
        ...
        avg_stmt = (
            select(func.avg(RiskForecast.escalation_probability_30d))
            .join(
                subq,
                and_(
                    RiskForecast.risk_id == subq.c.risk_id,
    ...
    gap_summary = db.execute(gap_summary_stmt, {"tenant_id": tenant_id}).mappings().first()
    ...
    task_row = db.execute(task_stmt, {"tenant_id": tenant_id}).mappings().first()
    ...
    try:
        dashboard_stmt = text(
            """
            SELECT total_evidences
            FROM analytics.v_dashboard_summary
            WHERE tenant_id = :tenant_id
            LIMIT 1
            """
        )
        dashboard_row = db.execute(dashboard_stmt, {"tenant_id": tenant_id}).mappings().first()
    ...
    health_index = 100.0 - (
        (float(gap_count) * 2.0)
        + (float(critical_risks) * 5.0)
        + (float(high_risks) * 2.0)
        + (float(avg_escalation_probability) * 20.0)
    )
    health_index = max(0.0, round(health_index, 2))
    ...
    for row in trend_rows:
        ...
        day_health_index = 100.0 - (
            (float(day_gap_count) * 2.0)
            + (day_worst_severity * 0.5)
        )
        day_health_index = max(0.0, round(day_health_index, 2))

        trend.append(
            {
                "day": row["day"].isoformat() if row.get("day") else None,
                "gap_count": day_gap_count,
                "worst_severity": day_worst_severity,
                "health_index": day_health_index,
            }
        )

    return {
        "linked_risks": int(linked_risks),
        "high_risks": int(high_risks),
        "critical_risks": int(critical_risks),
        "avg_escalation_probability": float(avg_escalation_probability),
