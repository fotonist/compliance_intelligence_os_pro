...
class ExecutiveSummaryService:
    ...
    def _dashboard_metrics(self) -> dict[str, Any]:
        ...
        if not row:
            raise RuntimeError(
                f"analytics.v_dashboard_summary returned no row "
    ...
    def control_metrics(self) -> dict[str, Any]:
        ...
        covered = int(dashboard["covered_controls"] or 0)
        partial = int(dashboard["partial_controls"] or 0)
        uncovered = int(dashboard["uncovered_controls"] or 0)
    ...
    def calculate_compliance_score(self) -> float:
        return float(
            self._dashboard_metrics()["compliance_health"] or 0.0
        )
    ...
    def risk_metrics(self) -> dict[str, Any]:
        ...
        exposure = float(
            dashboard["unified_exposure"] or 0.0
        )
    ...
    def calculate_risk_health(self) -> float:
        return round(
            max(
                0.0,
                100.0
                - self.risk_metrics()["average_score"] * 2.0,
            ),
            2,
        )
    ...
    def _evidence_state(
        ...
        files = [
            f
            for f in (
                getattr(evidence, "files", None) or []
            )
    ...
    def calculate_evidence_score(self) -> float:
        return float(
            self.evidence_metrics()["strength_percent"]
        )
    ...
    def weak_evidences(
        ...
        for evidence in evidences:
            state = self._evidence_state(evidence)
